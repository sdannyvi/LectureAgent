...
\textbf{Standard ML:} learn $f(x) \approx \E[Y \mid X=x]$.

\medskip
\textbf{Causal ML:} learn \emph{two} response functions:
\[
...
\textbf{Key constraint:} for each unit only \emph{one} arm is observed
($T\!\cdot\! Y(1) + (1{-}T)\!\cdot\! Y(0)$).
The other is the missing counterfactual.

\medskip
\textbf{Architecture (TARNet):} shared representation $\phi(X)$
followed by separate $T\!=\!0$ and $T\!=\!1$ prediction heads.
...
\begin{tikzpicture}[node distance=0.9cm, every node/.style={font=\small}]
  ...
  \node[font=\tiny, right=0.1cm of phi] {shared repr.};
\end{tikzpicture}
\end{center}
\end{columns}
\end{frame}
...
% -------------------------------------------------------
\begin{frame}{How are causal NNs trained?}

Training is \textbf{fully supervised, end-to-end} --- no unsupervised pre-training.

\begin{block}{Factual loss (only the observed arm contributes per unit)}
\[
\mathcal{L}(\theta)
= \sum_{i=1}^{n}
  \Bigl[
    T_i \cdot \ell\!\bigl(Y_i, \hat\mu_1(\phi(X_i))\bigr)
    +
    (1-T_i)\cdot \ell\!\bigl(Y_i, \hat\mu_0(\phi(X_i))\bigr)
  \Bigr]
\]
\end{block}

\begin{itemize}
  \item Each unit activates \emph{only one} head. $\phi$ is updated by
        backprop through the active head.
  \item The counterfactual head gets \emph{no direct gradient signal}
        from that unit.
\end{itemize}

\begin{alertblock}{Consequence}
$\phi$ is useful for factual prediction --- but may not generalise to
counterfactual prediction where no training counterparts exist in the
opposite arm. \textbf{This is exactly the non-overlap problem.}
\end{alertblock}
\end{frame}
...
\textbf{Digit 9} (50\% of data): $e=1/9$, so
8 in 9 nines are \emph{untreated} and 1 in 9 are \emph{treated}.\\
$\Rightarrow$ $\mu_0(9)$: estimated from many untreated nines.\\
$\Rightarrow$ $\mu_1(9)$: estimated from some (fewer) treated nines.\\
\textbf{Both arms have data.} CATE is \textbf{estimable}.
...
% -------------------------------------------------------
\begin{frame}{The problem and the paper's answer}

\begin{block}{Problem}
Neural CATE estimators extrapolate silently when positivity fails.
Propensity trimming uses a scalar $e(x)$ to decide estimability in a
high-dimensional space --- and gets it wrong (CEMNIST: removes estimable
nines, keeps non-estimable twos).
\end{block}

\smallskip
\begin{block}{Core insight}
Non-overlap $\equiv$ covariate shift for one arm model:\\[2pt]
$e(x)\approx 0 \;\Rightarrow\; x$ OoD for $\mu_1$
\quad
$e(x)\approx 1 \;\Rightarrow\; x$ OoD for $\mu_0$
\end{block}

\smallskip
\begin{block}{Solution}
Add epistemic uncertainty to the causal model (BNN / MC Dropout).\\
Uncertainty $U(x)$ is high exactly where OoD-ness occurs.\\
\textbf{Reject on $U(x)$, not on $\hat e(x)$.}
\end{block}
...
\textbf{Setting:} given budget $r_{\mathrm{rej}}$, withhold the
$r_{\mathrm{rej}}$ fraction with the worst signal.
\textbf{Metric:} error rate and $\sqrt{\mathrm{PEHE}}$ on non-rejected units.
...
\toprule
 & \textbf{Uncertainty rejection} & \textbf{Propensity trimming} \\
\midrule
Rejection rule
  & $U(x) > \kappa$
  & $\hat e(x) < \eta$ or $> 1{-}\eta$ \\
Signal type
  & model-based, arm-specific
  & marginal probability \\
Detects non-overlap
  & \textcolor{green!60!black}{Yes, arm-by-arm}
  & \textcolor{green!60!black}{Yes} \\
Detects covariate shift
  & \textcolor{green!60!black}{Yes}
  & \alert{No} \\
CEMNIST: keeps twos?
  & \alert{No} (high $U$, rejected)
  & \alert{No} (high $\hat e$, rejected) \\
CEMNIST: keeps nines?
  & \textcolor{green!60!black}{Yes} (low $U$)
  & \alert{No} (low $\hat e$, trimmed away) \\
